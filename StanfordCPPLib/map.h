 ...
 * If \em key is not found, this method returns the
 ...
 * value.  If key is not present in this map, a new entry is created
...
std::istream & operator>>(std::istream & is, Map<KeyType,ValueType> & map) {
   char ch;
   is >> ch;
   if (ch != '{') error("Map::operator >>: Missing {");
   map.clear();
   is >> ch;
   if (ch != '}') {
      is.unget();
      while (true) {
         ...
         is >> ch;
         if (ch != ':') error("Map::operator >>: Missing colon after key");
         ValueType value;

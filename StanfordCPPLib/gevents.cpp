...
string GEvent::toString() const {
   if (eventClass == 0) {
      ...
      return "GEvent(unknown eventClass)";
   }
}
...
string GActionEvent::getActionCommand() const {
   if (!valid) error("GActionEvent::getActionCommand: Event is not valid");
...
double GMouseEvent::getX() const {
   if (!valid) error("GMouseEvent::getX: Event is not valid");
   return x;
}

double GMouseEvent::getY() const {
   if (!valid) error("GMouseEvent::getY: Event is not valid");
   return y;
}
...
char GKeyEvent::getKeyChar() const {
   if (!valid) error("GKeyEvent::getKey: Event is not valid");
   return char(keyChar);

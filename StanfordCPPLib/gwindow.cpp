...
double GWindow::getCanvasWidth() {
  return pp->getCanvasWidth(*this);
}

double GWindow::getCanvasHeight() {
  return pp->getCanvasHeight(*this);
}
...
string convertRGBToColor(int rgb) {
   ...
   int aa = (rgb >> 24 & 0xFF);
   if (aa != 0xFF) os << setw(2) << aa;
   os << setw(6) << (rgb & 0xFFFFFF);
   return os.str();
}

...
ChangeListener {
	...
	protected void createWindow(String id, int width, int height,
		...
		class Runner implements Runnable {
			public Runner(JBEWindow window, JBETopCompound top) {
				this.window = window;
				this.top = top;
			}
			public void run() {
			...
			private JBEWindow window;
			private JBETopCompound top;
		}
		...
		window.pack();
		SwingUtilities.invokeLater(new Runner(window, top));
	...
	protected void setConsoleSize(int width, int height) {
		...
		SwingUtilities.invokeLater(new Runnable() {
	        public void run() {
	    		...
	    		if (consoleFrame != null)
	    			consoleFrame.setSize(new Dimension(consoleWidth, consoleHeight));
	        }
	    });	
	}
	...
	protected void putConsole(String str, boolean isStdErr) {
		if (consoleFrame == null) showConsole();
		console.print(str, isStdErr);
	}
	...
	public void windowClosing(WindowEvent e) {
		if (e.getSource() != consoleFrame) {
			...
			// Console was closed
			synchronized (eventLock) {
				System.out.println("event:consoleWindowClosed");
				System.out.flush();
				acknowledgeEvent();
			}
		}
	...
	private void printLog(String s) {
		try {
			if (out == null) {
				out = new PrintStream(new FileOutputStream("stanfordspldebug.txt"));
			}
			out.println(s);
			out.flush();
			//out.close();
	...
	protected Clip getClip(String name) {
		...
		try {
			...
			BufferedInputStream bis = new BufferedInputStream(is); // JL
			AudioInputStream ais = AudioSystem.getAudioInputStream(bis); // JL changed is to bis.
			AudioFormat format = ais.getFormat(); // JL
			DataLine.Info info = new DataLine.Info(Clip.class, format); // JL
			clip = (Clip) AudioSystem.getLine(info);  // JL
			clip.open(ais);
		} catch (IOException ex) {
